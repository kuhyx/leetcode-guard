...
## Solving buys gaming time and a later shutdown

`screen-locker` moves the nightly shutdown **+1h** later and
`steam-backlog-enforcer` adds **+1h** of gaming budget on a day with an
accepted submission. Both read `ledger.json` and fail closed; leetcode-guard
writes neither reward. Details, the endpoint and the staleness caveat:
[`DOCS-solved-today-consumers.md`](./DOCS-solved-today-consumers.md).
