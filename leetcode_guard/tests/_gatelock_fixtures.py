"""Keep gatelock's per-output machinery off the real machine.

Patching ``tk`` inside :mod:`leetcode_guard` is not enough. Since gatelock
v0.2.0 ``LockWindow`` builds one real ``tk.Toplevel`` per live output through
*gatelock's own* ``tk``. Over a MagicMock root that sends real tkinter into
unbounded mock recursion, so the suite **hangs** instead of failing -- far
harder to diagnose than an error, and exactly what this module prevents.

Copied from screen-locker, which hit the hang first.
"""
...
"""The real desk. Opt in with ``dual_output`` for anything asserting that work
fans out across monitors: against ``FAKE_OUTPUTS`` a loop that only ever
touched the primary passes every assertion."""
...
def _hermetic_gatelock(
    ...
    The runtime dir is redirected too, so a test can never reach the real
...
def dual_output(_hermetic_gatelock: None) -> Iterator[None]:
    ...
    Depends on ``_hermetic_gatelock`` so pytest applies it second; the
    ``scan_xrandr`` patch here has to be the one that wins.
    """
