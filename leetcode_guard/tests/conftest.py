...
Deliberately separate from ``_TK_MODULES``: that list is defined by
...
def _fake_gatelock_widgets(monkeypatch: pytest.MonkeyPatch, fake: MagicMock) -> None:
...
def _block_real_tk(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    for name in _TK_MODULES:
        module = importlib.import_module(name)
        monkeypatch.setattr(module, "tk", fake, raising=True)
    _fake_gatelock_widgets(monkeypatch, fake)
