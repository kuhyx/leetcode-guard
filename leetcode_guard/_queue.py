...
The lockers form a ladder, and gatelock's own module docstring already names
it: wake up, then work out, then grind, then eat. What was missing was the
waiting half. Today nothing calls ``Arbiter.evaluate()`` -- every consumer
publishes a claim, calls ``acquire_holder()``, discards the result and builds
its window anyway. The loser draws a visible, ungrabbed window *behind* the
winner and spins in gatelock's forever-retrying grab loop.

So this waits **headlessly**: no root, no surfaces, nothing on screen, until no
live claim outranks us. Then the caller arms normally.
...
from leetcode_guard._constants import QUEUE_DEADLINE_SECONDS, QUEUE_POLL_SECONDS

if TYPE_CHECKING:
    from collections.abc import Callable

    from gatelock import Arbiter, Claim
    ...
    *,
    ...
    while True:
        ...
        if not blockers:
            ...
            if seen:
                _logger.warning(
                    "leetcode-guard waited %.0fs behind %s; arming now",
                    waited,
                    ", ".join(seen),
                )
            return QueueResult(
        ...
        for claim in blockers:
            if claim.app not in seen:
                ...
                _logger.info(
                    "leetcode-guard is queued behind %s (rank %d) -- waiting "
                    "with no window",
                    claim.app,
                    claim.rank,
                )
        ...
        if elapsed >= deadline:
            _logger.error(
                "leetcode-guard waited %.0fs behind %s and gave up waiting -- "
                "arming anyway rather than leaving the machine unlocked",
                elapsed,
                ", ".join(seen),
            )
            return QueueResult(
